...
function ResultCard(props: ResultCardProps) {
    ...
    switch (props.status) {
        case HeatStatus.BREAK:
            return (
                BreakResultCard(props)
            )

    }
    return (
        <div
            className={`transition-all duration-700 ${expanded ? `w-full h-[500px]` : `w-[405px] h-[413px]`}  px-[15px] pt-2.5 pb-[15px] bg-neutral-50 rounded-[20px] border border-zinc-800 border-opacity-20 flex-col justify-start items-start gap-2.5 inline-flex`}>
            {
                expanded ? (
